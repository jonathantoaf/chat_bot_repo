...
class FileSystemClient:
    ...
    def save_audio_file(self, audio_file: UploadFile, audio_file_path: Path) -> None:
        try:
            ...
            self.__logger.error(f"An error occurred: {str(e)}. when try to save the audio file in this path :{audio_file_path}")
    ...
    def open_audio_file(self, audio_file_path: Path) -> Tuple[Any, int]:
        try:
            ...
            self.__logger.error(f"An error occurred: {str(e)}. when try to open the audio file in this path :{audio_file_path}")
            raise e